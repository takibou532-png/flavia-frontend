...
const navItems = [
  ...
  {id: 2, label: 'Menu', icon: <RestaurantRoundedIcon className="w-5 h-5" /> },
  {id: 3, label: 'Categories', icon: <CategoryOutlinedIcon className="w-5 h-5" />},
  {id:4 ,label:'Orders',icon : <ListOrdered className="w-5 h-5"/>},
  {id:5 ,label:'Reservations',icon :<BookmarkBorderOutlinedIcon className="w-5 h-5"/>},
  {id:6 ,label:'Employees',icon :<EngineeringIcon className="w-5 h-5"/>},
...
>
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-2">
        ...
        {navItems.map((item) => (
         <Link  key={item.id} to={`/dashboard/${item.label}`}>
         <button
